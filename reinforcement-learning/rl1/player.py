...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        # Init a numpy array with ns state rows and na cols w/ zeros
        num_actions = np.zeros((ns,na))
        # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                if np.isnan(s_current):
                    printerr("NAAAAN")
                num_actions[s_current, action] += 1
            ...
            episode += 1
            end_episode = False
        printerr("NUM_ACTION = " + str(num_actions))
        #policy = np.argmax(num_actions, axis=1)
        #printerr("POLICY = " + str(policy))
    ...
    def get_policy(self, Q):
        ...
        for _ in range(len(Q)):
            try:
                ...
                #printerr(str(i) + " USING argmax")
            except:
                nan_max_actions_proxy[_] = np.random.choice([0, 1, 2, 3])
                #printerr(str(i) + " USING RANDOM")
            i += 1

        #printerr("NAN_MAX = " + str(nan_max_actions_proxy))
